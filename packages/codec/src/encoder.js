...
import { PAYLOAD_VERSION, BUDGET_CHARS, MAX_TITLE_CHARS, VIEWER_ORIGIN, VIEWER_PATH, } from "./constants.js";
/**
 ...
 */
export function createPayload(tabs) {
    const now = Math.floor(Date.now() / 1000);
    const expiry = now + 24 * 3600; // 24 hours
    return {
        v: PAYLOAD_VERSION,
        e: expiry,
        i: tabs.map((tab) => [tab.url, normalizeTitle(tab.title)]),
    };
}
/**
 ...
 */
export async function findMaxTabsWithinBudget(tabs) {
    if (tabs.length === 0)
        return 0;
 ...
 */
export async function encodeTabsToShareUrl(tabs) {
    if (tabs.length === 0) {
        return {
            url: buildShareUrl(""),
            itemCount: 0,
