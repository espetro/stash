...
| Does an MV3 extension SW hit the Local Network Access prompt for `127.0.0.1`? | **Resolved (spiked 2026-08-30, spec §12.2 row updated in plan F1.W5 outcome).** Unpacked MV3 SW `fetch`ing `http://127.0.0.1:17777/` from the SW at cold start and on timers: with `host_permissions: ["http://127.0.0.1/*"]` the fetch succeeds (`FETCH_OK status=200` at cold start, +5s, +10s) with no LNA prompt observed; with the pattern in `permissions` only, or with no permission at all, the fetch fails immediately (`Failed to fetch`) on every attempt. Tested on Chrome for Testing 151.0.7922.34 (branded Chrome 152 refuses `--load-extension`, logs `--load-extension is not allowed in Google Chrome, ignoring.`). Conclusion: loopback fetches from an MV3 SW need `host_permissions` (not plain `permissions`) but no interactive LNA grant on this build. Spike dir: `spikes/lna-loopback/`. |
